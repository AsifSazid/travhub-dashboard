...
--     sys_id format: THR-26-CNT-01-TRS-01-TRV-01client_payments
...
-- ═══════════════════════════════════════════════════════════════════
-- TravHub — Step 1 Alterations
-- Run this ONCE on your existing travhub-packages database
-- ═══════════════════════════════════════════════════════════════════

-- ─────────────────────────────────────────────────────────────────
-- transport_variants: add seat_count, max_luggage_kg, max_luggage_bags
-- replaces the old free-text luggage_capacity column
-- ─────────────────────────────────────────────────────────────────
ALTER TABLE `transport_variants`
    ADD COLUMN `seat_count`       TINYINT UNSIGNED NULL     COMMENT 'Number of seats in vehicle'
        AFTER `capacity_max`,
    ADD COLUMN `max_luggage_kg`   SMALLINT UNSIGNED NULL    COMMENT 'Max luggage weight in KG'
        AFTER `seat_count`,
    ADD COLUMN `max_luggage_bags` TINYINT UNSIGNED NULL     COMMENT 'Max luggage bags count'
        AFTER `max_luggage_kg`;
