<?php
/**
 * api/reports/payment/endpoints.php
 *
 * Action-routed API for the Payments Report.
 * Source table: ac_banking_stmts (related_type = 2 => payment)
 *
 * Actions (?action=...):
 ...
 *
 * Pattern: zero PDO in view files — this is the only file that touches DB
 * for the payment report.
 */
...
try {
    ...
    echo json_encode([
        'success' => false,
        'error'   => 'Server error in payment report endpoint.',
...
function handleList(PDO $pdo, bool $isExport): void
{
    ...
    $sumStmt = $pdo->prepare("
        SELECT
            ...
            COALESCE(SUM(CAST(bs.withdraw AS DECIMAL(15,2))), 0) AS total_amount
        {$baseSql}
    ");
    ...
    $selectCols = "
        ...
        bs.withdraw AS amount, bs.balance, bs.reconsilation, bs.ref
    ";
...
{
    $where  = ["bs.related_type = 2"]; // payment only
    $params = [];
    ...
    if (!empty($_GET['amount_min'])) {
        $where[] = "CAST(bs.withdraw AS DECIMAL(15,2)) >= :amount_min";
        $params[':amount_min'] = $_GET['amount_min'];
    }
    if (!empty($_GET['amount_max'])) {
        $where[] = "CAST(bs.withdraw AS DECIMAL(15,2)) <= :amount_max";
