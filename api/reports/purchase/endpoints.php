<?php
/**
 * api/reports/purchase/endpoints.php
 *
 * Action-routed API for the Purchase Report.
 * Source table: financial_entries (related_type = 2 => purchase, user_type = 'vendor')
 *
 * Actions (?action=...):
...
try {
    ...
    echo json_encode([
        'success' => false,
        'error'   => 'Server error in purchase report endpoint.',
...
function handleFilters(PDO $pdo): void
{
    $vendorStmt = $pdo->query("
        ...
        WHERE related_type = 2 AND user_type = 'vendor' AND user_sys_id IS NOT NULL
    ...
    $workStmt = $pdo->query("
        ...
        FROM financial_entries
        WHERE related_type = 2 AND work_sys_id IS NOT NULL
...
{
    $where  = ["fe.related_type = 2", "fe.user_type = 'vendor'"]; // purchase only
    $params = [];
