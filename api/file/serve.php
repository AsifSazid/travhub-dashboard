...
// ?conf_id=C-001&task_id=THR-A26-TK-0002&page=0  (task_id resolves to its work internally)
// Confirmation files are stored in at_confirmations[].files_json, each entry
// an object {name, file_name, smb_token, mime_type, uploaded_at, uploaded_by, extracted_data}
function _serveConf(string $confId, bool $dl): void
{
    ...
    $workId = $_GET['work_id'] ?? '';
    if (!$taskId && !$workId) {
        ob_clean(); http_response_code(400); echo 'task_id or work_id required for conf_id'; return;
    }

    // air_tickets rows are keyed by work_sys_id, not task_sys_id — resolve
    // the work first if only a task id was given.
    if (!$workId && $taskId) {
        $tw = $pdo->prepare("SELECT work_sys_id FROM tasks WHERE sys_id = ? LIMIT 1");
        $tw->execute([$taskId]);
        $workId = $tw->fetchColumn() ?: '';
    }
    if (!$workId) {
        ob_clean(); http_response_code(404); echo 'Could not resolve work for this task'; return;
    }

    $stmt = $pdo->prepare("SELECT at_confirmations FROM air_tickets WHERE work_sys_id = ? LIMIT 1");
    $stmt->execute([$workId]);
    ...
    // Each entry's smb_token field is a FULL URL (smbFileUrl()'s return value),
    // not a raw token — extract the token from its query string.
    $rawSmbToken = is_array($entry) ? ($entry['smb_token'] ?? null) : $entry;
    if (!$rawSmbToken) {
        ob_clean(); http_response_code(404); echo 'File token missing'; return;
    }
    $urlParts = parse_url($rawSmbToken);
    if (isset($urlParts['query'])) {
        parse_str($urlParts['query'], $qs);
        $token = $qs['smb_token'] ?? $rawSmbToken; // fall back if it wasn't a URL after all
    } else {
        $token = $rawSmbToken; // plain token string (older rows)
    }
