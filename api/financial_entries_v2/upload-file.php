...
// FILE PATH: api/financial_entries_v2/upload-file.php
/**
 * FILE PATH: /api/financial_entries_v2/upload-file.php
...
// Get client info from works (current schema — client_info is JSON, not a FK column)
$clientSysIdRaw = null;
$clientNameRaw  = null;
try {
    $stmt = $pdo->prepare("SELECT client_info FROM works WHERE sys_id = ? LIMIT 1");
    $stmt->execute([$feWorkSysId]);
    $ci = json_decode($stmt->fetchColumn() ?: '{}', true) ?? [];
    $clientSysIdRaw = $ci['sys_id'] ?? null;
    $clientNameRaw  = $ci['name']   ?? null;
} catch (Exception $e) {
    // leave nulls — fallback below
}

$clientSysId = preg_replace('/\s+/u', '', $clientSysIdRaw ?? 'UNKNOWN');
$clientWords = preg_split('/[\s_]+/', trim($clientNameRaw ?? 'Unknown'));
