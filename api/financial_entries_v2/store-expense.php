<?php
// PATH: /api/financial_entries_v2/store-expense.php
// (v2 -- office operating expense, double-entry)
//
// Records an office expense (rent, utility, internet, marketing, supplies,
// etc.), fully double-entry:
//   Row 1: account_head=expense,      debit  (expense recognized)
//   Row 2: account_head=bank_account, credit (+ac_banking_stmts, money out
//          of a real transactionable account -- Bank/Cash/Petty Cash)
//
// POST { expense_account_id, payment_account_id, amount, purpose, date, ref? }
//   expense_account_id -- an ac_banking row in category Expenses/Cost of
//                         sales/Other Expense (classification only, see
...
try {
    ...
    $expenseAccountId = trim($input['expense_account_id'] ?? '');
    ...
    $errors = [];
    if (!$expenseAccountId) $errors[] = 'expense_account_id is required';
    ...
    /* ================= Lookup + validate the expense (classification) account ================= */
    $expStmt = $pdo->prepare("SELECT acc_name, category FROM ac_banking WHERE sys_id = ?");
    $expStmt->execute([$expenseAccountId]);
    $expenseAccount = $expStmt->fetch(PDO::FETCH_ASSOC);
    if (!$expenseAccount) throw new Exception('Expense account not found');

    $resolvedHead = resolveAccountHeadFromCategory($expenseAccount['category']);
    if ($resolvedHead !== 'expense') {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => "The selected account is not an Expense-category account (category: {$expenseAccount['category']})"]);
        exit;
    }
    $expenseAccountName = $expenseAccount['acc_name'];
    ...
    // Expense: debit (expense recognized)
    $expenseEntrySysId = _insertFinancialEntry($pdo, array_merge($baseRow, [
        'user_sys_id' => $expenseAccountId, 'user_name' => $expenseAccountName, 'user_type' => 'account',
        'account_head' => 'expense', 'vendor_type' => null,
    ...
    postBankLegV2($pdo, $paymentAccountId, $paymentAccountName, $oldBalance, $amount, 'out', $date,
        "{$expenseAccountName} — {$purpose}", $bankEntrySysId, $userName,
        $paymentMethod, $expenseAccountId, $expenseAccountName, 'account', $instrumentNo);
    ...
    echo json_encode([
        ...
        'expense_entry_sys_id' => $expenseEntrySysId,
