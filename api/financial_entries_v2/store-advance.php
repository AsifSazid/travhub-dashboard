<?php
// PATH: /api/financial_entries_v2/store-advance.php
// (v2 -- task-bound, per user's explicit instruction that advance/discount/
//  gratuity must always reference a specific task)
//
// Records an ADVANCE received from a client before a sale/service is fully
// delivered/billed. This creates a liability (we owe the client either the
// service or a refund) tracked separately from accounts_receivable.
//
// POST { client_id, work_id, task_id, account_id, amount, date, purpose?, payment_method?, instrument_no? }
//   account_id -- the transactionable Bank/Cash account the advance money
//                 actually landed in
//
// Creates a NEW transaction group with:
//   Row 1: account_head=advance_received, credit (liability increases --
//          we now owe the client this amount in service or refund)
//   Row 2: account_head=bank_account,     debit  (+ac_banking_stmts, money in)
//
// task_id/work_id are REQUIRED (not optional) -- an advance with no task
// context has no way to later be applied against a specific sale.
...
try {
    ...
    if (!$workId)    $errors[] = 'work_id is required — advance must be tied to a task';
    if (!$taskId)    $errors[] = 'task_id is required — advance must be tied to a task';
    ...
    $finalPurpose = $purpose ?: "Advance received from {$clientName}";
    ...
    $advanceEntrySysId = _insertFinancialEntry($pdo, array_merge($baseRow, [
        'user_sys_id' => $clientId, 'user_name' => $clientName, 'user_type' => 'client',
        'account_head' => 'advance_received', 'vendor_type' => null,
    ...
    echo json_encode([
        ...
        'advance_entry_sys_id' => $advanceEntrySysId,
