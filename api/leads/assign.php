...
try {
    ...
    if ($assignedTo && !empty($assignedTo['sys_id'])) {
        // Fetch lead client name for notification body
        $leadStmt = $pdo->prepare("SELECT client_info FROM leads WHERE sys_id = ? LIMIT 1");
        ...
        $pdo->prepare("
            ...
            VALUES
                (?, ?, 'user', ?, ?, 'lead_assigned', ?, ?, NULL, NULL, NULL, 0, ?)
        ")->execute([
            $ntIds['uuid'], $ntIds['sys_id'],
            'system_test', $assignedTo['sys_id'],
