...
$parent = trim($_GET['activity_sys_id'] ?? '');
if ($parent) { $where[] = "activity_sys_id = :pf"; $params[':pf'] = $parent; }
    $w = $where ? 'WHERE '.implode(' AND ', $where) : '';
    $cnt = $pdo->prepare("SELECT COUNT(*) FROM activity_variants $w");
    $cnt->execute($params); $total = (int)$cnt->fetchColumn();
    $stmt = $pdo->prepare("SELECT * FROM activity_variants $w ORDER BY variant_name ASC LIMIT :lim OFFSET :off");
