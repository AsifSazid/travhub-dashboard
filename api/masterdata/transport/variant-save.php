...
//         luggage_capacity?, price_basis, transfer_type, meet_and_greet,
...
$luggage_capacity=trim($in['luggage_capacity']??'');
...
if (!$service_sys_id||!$variant_name||$net_cost<=0){echo json_encode(['success'=>false,'message'=>'service_sys_id, variant_name, net_cost required']);exit;}
try {
    $isNew=empty($sys_id);
    if ($isNew) {
        ...
        $pdo->prepare("INSERT INTO transport_variants (uuid,sys_id,service_sys_id,country_sys_id,car_sys_id,variant_name,vehicle_class,capacity_max,luggage_capacity,price_basis,transfer_type,meet_and_greet,currency_code,net_cost,markup_type,markup_value,sell_price,child_price,extra_charges,status,meta_data)
            VALUES(:uuid,:sid,:ssid,:csid,:carsid,:vname,:vc,:cmax,:lc,:pb,:tt,:mg,:cc,:nc,:mt,:mv,:sp,:cp,:ec,'active',:meta)")
        ->execute([':uuid'=>$uuid,':sid'=>$sys_id,':ssid'=>$service_sys_id,':csid'=>$country_sys_id,
            ':carsid'=>$car_sys_id?:null,':vname'=>$variant_name,
            ':vc'=>$vehicle_class,':cmax'=>$capacity_max,':lc'=>$luggage_capacity?:null,
            ':pb'=>$price_basis,':tt'=>$transfer_type,':mg'=>$meet_and_greet,
            ':cc'=>$currency_code,':nc'=>$net_cost,':mt'=>$markup_type,':mv'=>$markup_value,
            ':sp'=>$sell_price,':cp'=>$child_price,
            ':ec'=>json_encode($extra_charges,JSON_UNESCAPED_UNICODE),':meta'=>$meta]);
        ...
        $pdo->prepare("UPDATE transport_variants SET car_sys_id=:carsid,variant_name=:vname,vehicle_class=:vc,capacity_max=:cmax,luggage_capacity=:lc,price_basis=:pb,transfer_type=:tt,meet_and_greet=:mg,currency_code=:cc,net_cost=:nc,markup_type=:mt,markup_value=:mv,sell_price=:sp,child_price=:cp,extra_charges=:ec,meta_data=:meta WHERE sys_id=:sid")
        ->execute([':carsid'=>$car_sys_id?:null,':vname'=>$variant_name,
            ':vc'=>$vehicle_class,':cmax'=>$capacity_max,':lc'=>$luggage_capacity?:null,
            ':pb'=>$price_basis,':tt'=>$transfer_type,':mg'=>$meet_and_greet,
            ':cc'=>$currency_code,':nc'=>$net_cost,':mt'=>$markup_type,':mv'=>$markup_value,
            ':sp'=>$sell_price,':cp'=>$child_price,
            ':ec'=>json_encode($extra_charges,JSON_UNESCAPED_UNICODE),':meta'=>$meta,':sid'=>$sys_id]);
