...
$parent = trim($_GET['country_sys_id'] ?? '');
if ($parent) { $where[] = "country_sys_id = :pf"; $params[':pf'] = $parent; }
    $w = $where ? 'WHERE '.implode(' AND ', $where) : '';
    $cnt = $pdo->prepare("SELECT COUNT(*) FROM transport_services $w");
    $cnt->execute($params); $total = (int)$cnt->fetchColumn();
    $stmt = $pdo->prepare("SELECT * FROM transport_services $w ORDER BY name ASC LIMIT :lim OFFSET :off");
