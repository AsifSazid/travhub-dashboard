...
$sys_id=trim($in['sys_id']??''); $restore=!empty($in['restore']);
if (!$sys_id){echo json_encode(['success'=>false,'message'=>'sys_id required']);exit;}
try {
    $row=$pdo->prepare("SELECT name,meta_data FROM cars WHERE sys_id=? LIMIT 1");
    ...
    $pdo->prepare("UPDATE cars SET status=?,meta_data=? WHERE sys_id=?")->execute([$restore?'active':'deleted',$meta,$sys_id]);
    syncCarsJson($pdo);
    $action=$restore?'restored':'deleted';
    echo json_encode(['success'=>true,'action'=>$action,'message'=>"Car '{$existing['name']}' {$action}."]);
