...
$parent = trim($_GET['room_type_sys_id'] ?? '');
if ($parent) { $where[] = "room_type_sys_id = :pf"; $params[':pf'] = $parent; }
    $w = $where ? 'WHERE '.implode(' AND ', $where) : '';
    $cnt = $pdo->prepare("SELECT COUNT(*) FROM room_rates $w");
    $cnt->execute($params); $total = (int)$cnt->fetchColumn();
    $stmt = $pdo->prepare("SELECT * FROM room_rates $w ORDER BY valid_from DESC LIMIT :lim OFFSET :off");
