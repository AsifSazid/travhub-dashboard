...
$pdo->prepare("INSERT INTO hotels (uuid,sys_id,country_sys_id,country_name,city_sys_id,city_name,name,search_terms,star_rating,address,phone,email,description,amenities,images,check_in_time,check_out_time,status,meta_data) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'active',?)")            ->execute([$uuid,$sys_id,$country_sys_id,$country_name?:null,$city_sys_id?:null,$city_name?:null,$name,$search_terms?:null,$star_rating,$address?:null,$phone?:null,$email?:null,$description?:null,json_encode($amenities,JSON_UNESCAPED_UNICODE),json_encode($images,JSON_UNESCAPED_UNICODE),$check_in_time?:null,$check_out_time?:null,$meta]);
        syncHotelsJson($pdo);
        ...
        $pdo->prepare("UPDATE hotels SET country_name=?,city_sys_id=?,city_name=?,name=?,search_terms=?,star_rating=?,address=?,phone=?,email=?,description=?,amenities=?,images=?,check_in_time=?,check_out_time=?,meta_data=? WHERE sys_id=?")
            ->execute([$country_name?:null,$city_sys_id?:null,$city_name?:null,$name,$search_terms?:null,$star_rating,$address?:null,$phone?:null,$email?:null,$description?:null,json_encode($amenities,JSON_UNESCAPED_UNICODE),json_encode($images,JSON_UNESCAPED_UNICODE),$check_in_time?:null,$check_out_time?:null,$meta,$sys_id]);
        syncHotelsJson($pdo);
