...
$parent = trim($_GET['hotel_sys_id'] ?? '');
if ($parent) { $where[] = "hotel_sys_id = :pf"; $params[':pf'] = $parent; }
    $w = $where ? 'WHERE '.implode(' AND ', $where) : '';
    $cnt = $pdo->prepare("SELECT COUNT(*) FROM room_types $w");
    $cnt->execute($params); $total = (int)$cnt->fetchColumn();
    $stmt = $pdo->prepare("SELECT * FROM room_types $w ORDER BY room_name ASC LIMIT :lim OFFSET :off");
