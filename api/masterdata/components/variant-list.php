...
$parent = trim($_GET['component_sys_id'] ?? '');
if ($parent) { $where[] = "component_sys_id = :pf"; $params[':pf'] = $parent; }
    $w = $where ? 'WHERE '.implode(' AND ', $where) : '';
    $cnt = $pdo->prepare("SELECT COUNT(*) FROM component_variants $w");
    $cnt->execute($params); $total = (int)$cnt->fetchColumn();
    $stmt = $pdo->prepare("SELECT * FROM component_variants $w ORDER BY variant_name ASC LIMIT :lim OFFSET :off");
