<div class="bg-gray-900 text-gray-200 flex flex-col overflow-x-hidden">
    <!-- Title Bar -->
    <div id="workFiles" class="bg-gray-800 h-10 grid grid-cols-3 items-center px-5 border-b border-gray-700">
        <!-- LEFT -->
        <div class="flex items-center gap-4">
            <div class="flex gap-2">
                <div class="w-3 h-3 rounded-full bg-red-500 cursor-pointer" onclick="closeExplorer()"></div>
                <div class="w-3 h-3 rounded-full bg-yellow-500 cursor-pointer" onclick="minimizeExplorer()"></div>
                <div class="w-3 h-3 rounded-full bg-green-500 cursor-pointer" onclick="maximizeExplorer()"></div>
            </div>
        ...
        <!-- CENTER -->
        <div class="flex items-center justify-center gap-2 text-gray-300 text-sm">
        ...
        <div class="flex items-center gap-4 justify-end">
            <i class="fas fa-search text-gray-400 cursor-pointer hover:text-white" onclick="searchFiles()"></i>
            <i class="fas fa-ellipsis-v text-gray-400 cursor-pointer hover:text-white" onclick="toggleMoreMenu()"></i>
        </div>
    </div>
    ...
    <div class="flex-1 p-5">
        <div class="bg-gray-800 rounded-lg overflow-hidden flex flex-col h-full shadow-2xl border border-gray-700">
            ...
            <div class="bg-gray-900 px-4 py-2 flex items-center gap-3 border-b border-gray-700">
                ...
                <div class="flex-1 bg-gray-800 border border-gray-600 px-4 py-2 rounded text-sm font-mono text-gray-300 truncate"
                    id="current-path-display" title="\storage\clients\">
...
<script>
    ...
    class FileExplorer {
        ...
        async loadFolder(path = '') {
            try {
                ...
                if (data.success) {
                    ...
                    this.updatePathDisplay(data.displayPath);
        ...
        updatePathDisplay(displayPath = '') {
            ...
            if (displayPath) {
                // Use the display path from API
                path = displayPath;
            } else {
                // Fallback: construct the path
                const basePath = `\\storage\\clients\\${this.clientFolder}\\${this.workFolder}`;
                const subPath = this.currentPath ? '\\' + this.currentPath.replace(/\//g, '\\') : '';
    ...
    function toggleMoreMenu() {
        alert('More menu to be implemented');
    }

    function closeExplorer() {
        if (confirm('Close File Explorer?')) {
            alert('File Explorer closed');
        }
    }

    function minimizeExplorer() {
        alert('File Explorer minimized');
    }

    function maximizeExplorer() {
        alert('File Explorer maximized');
    }
