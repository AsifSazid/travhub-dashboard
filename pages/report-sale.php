...
<head>
    ...
    <title>Sale Report | TravHub Global Limited</title>
...
<body class="bg-gray-50 text-navy">
    ...
    <main id="mainContent" class="pt-16 pl-0 lg:pl-64 lg:my-16 transition-all duration-300 h-full">
        <div class="max-w-7xl mx-auto p-4 md:p-6">
          ...
          <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
            <div>
              <h1 class="text-xl md:text-2xl font-semibold text-navy">Sale Report</h1>
              <p class="text-sm text-gray-500">Source: financial_entries &middot; related_type = sale (client)</p>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div class="bg-white rounded-xl border border-gray-100 p-4">
              <p class="text-xs text-gray-400 mb-1">Total Sale Entries (filtered)</p>
              <p id="sumCount" class="text-2xl font-semibold text-navy">0</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-100 p-4">
              <p class="text-xs text-gray-400 mb-1">Total Sale Amount (filtered)</p>
          ...
          <div class="bg-white rounded-xl border border-gray-100 p-4 mb-3">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-3">
              ...
              <div class="md:col-span-2">
                <label class="text-xs text-gray-500">Search (purpose / ref / client / work / sys_id)</label>
            ...
            <div id="advancedPanel" class="hidden mt-4 pt-4 border-t border-gray-100 grid grid-cols-1 md:grid-cols-4 gap-3">
              <div>
                ...
                <select id="f_client" multiple class="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 text-sm h-24"></select>
              </div>
              <div>
    ...
    (function () {
      ...
      const API = '/api/reports/sale/endpoints.php';
      ...
      async function loadFilterOptions() {
        try {
          ...
          document.getElementById('f_client').innerHTML = data.clients.map(c =>
            `<option value="${c.user_sys_id}">${c.user_name}</option>`
          ).join('');
      ...
      function buildParams(action, scopeAll) {
        ...
        if (!scopeAll) {
          ...
          [...document.getElementById('f_client').selectedOptions].forEach(o => p.append('client[]', o.value));
      ...
      function renderTable(rows) {
        ...
        if (!rows.length) {
          tbody.innerHTML = `<tr><td colspan="8" class="text-center py-6 text-gray-400">No sale entries found.</td></tr>`;
          return;
        }
      ...
      async function runExport(type, scope) {
        ...
        try {
          ...
          const filenameBase = `sale-report-${scope}-${new Date().toISOString().slice(0,10)}`;
      ...
      function exportPDF(rows, filenameBase, summary) {
        ...
        doc.autoTable({
          startY: 26,
          head: [['Date', 'Sys ID', 'Client', 'Work', 'Purpose', 'Ref', 'Amount', 'Paid', 'Discounted']],
          body: rows.map(r => [r.Date, r['Sys ID'], r.Client, r.Work, r.Purpose, r.Ref, r.Amount, r.Paid, r.Discounted]),
          styles: { fontSize: 7 },
