...
<head>
  ...
  <title>TravHub — Investor Accounting System</title>
  ...
  <!-- Custom CSS — শুধু যেটুকু Tailwind দিয়ে সম্ভব না -->
    <link rel="stylesheet" href="../assets/css/investors-dashboard.css" >
</head>
...
<aside id="sidebar" class="w-64 bg-navy flex flex-col flex-shrink-0 min-h-screen sticky top-0">
  ...
  <div class="px-5 py-[22px] border-b border-white/[0.07]">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <div class="text-white/35 text-[10px] font-medium tracking-wide">Investor Portal</div>
      </div>
    </div>
  </div>
  ...
  <nav class="flex-1 px-3 py-4 overflow-y-auto">
    ...
    $navItems = [
      ...
      ['id'=>'settings',   'label'=>'Investor Settings', 'icon'=>'<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>'],
      ['id'=>'profiles',   'label'=>'Investor Profiles', 'icon'=>'<circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/>'],
    ];
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center min-w-0 gap-2.5">
        ...
        <div class="overflow-hidden">
          ...
          <p class="text-[#9ca3af] text-xs truncate">Managing Investor</p>
        </div>
      </div>
...
<div class="flex-1 flex flex-col min-h-screen overflow-hidden">
  ...
  <header class="bg-white border-b border-[#e8eaf0] px-5 py-[13px] flex items-center justify-between sticky top-0 z-50">
    <div class="flex items-center gap-3">
      ...
      <div>
        <div id="pgTitle" class="text-[20px] font-bold text-navy leading-tight">Investor's Dashboard</div>
        <div id="pgSub" class="text-xs text-navy/45 font-normal">Overview of all investor accounts</div>
      </div>
    </div>
  ...
  <main class="flex-1 overflow-y-auto p-5 lg:p-6">
    ...
    <div id="page-dashboard" class="page active">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        ...
        <div class="bg-navy rounded-2xl p-5 shadow-sm col-span-2 lg:col-span-1">
          ...
          <div class="text-white/40 text-xs mb-3">All investors combined</div>
        ...
        <div class="bg-white rounded-2xl border border-[#e8eaf0] p-5 shadow-sm">
          <div class="text-navy/45 text-[11px] font-semibold mb-1 uppercase tracking-wide">Total Investors</div>
          <div id="d-tot-dir" class="text-[22px] font-bold text-navy mb-0.5">—</div>
          <div class="text-navy/40 text-xs">Active investors</div>
        </div>
    ...
    <div id="page-settings" class="page">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-5">
          ...
          <div class="bg-white rounded-2xl border border-[#e8eaf0] shadow-sm p-6">
            <div id="form-title" class="text-[16px] font-bold text-navy mb-5">Add Investor</div>
            ...
            <button onclick="saveDir()" id="save-btn" class="w-full bg-brand text-white rounded-[10px] font-semibold text-[13px] py-2.5 border-none cursor-pointer hover:bg-brand-dark transition-colors flex items-center justify-center gap-2">
              <span id="save-lbl">Add Investor</span>
            </button>
          ...
          <div class="bg-white rounded-2xl border border-[#e8eaf0] shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b border-[#f0f2f7]">
              <div class="text-[16px] font-bold text-navy">Investors</div>
    ...
    <div id="page-profiles" class="page">
      <div class="grid grid-cols-3 gap-4 mb-5">
        ...
        <div class="bg-white rounded-2xl border border-[#e8eaf0] shadow-sm p-5">
          <div class="text-[11px] font-semibold text-navy/45 mb-1 uppercase tracking-wide">Investors</div>
          ...
          <table>
            <thead>
              <tr>
                ...
                <th class="th-center">Investors</th>
                <th class="th-left">Note</th>
              </tr>
            </thead>
...
<div id="m-delete" class="modal-bg" onclick="if(event.target===this)closeModal('m-delete')">
  <div class="modal-box">
    ...
    <div class="text-[18px] font-bold text-navy mb-2">Deactivate Investor?</div>
...
const API = {
  investors: {
    getAll:  '../api/investors/get-all.php',
    create:  '../api/investors/store-from-dashboard.php',
    update:  '../api/investors/update.php',
    delete:  '../api/investors/delete.php',
    summary: '../api/investors/summary.php',
    details: id => `../api/investors/details.php?id=${id}`,
  },
  tx: {
    get:      id => `../api/investor_transactions/get.php?investor_id=${id}`,
    invest:   '../api/investor_transactions/invest.php',
    withdraw: '../api/investor_transactions/withdraw.php',
  },
  pl: {
...
// ── Navigation ───────────────────────────────────────────────────────────────
const pageTitles = {
  dashboard: ["Investor's Dashboard",'Overview of all investor accounts'],
  settings:  ['Investor Settings','Add or manage investor accounts'],
  profiles:  ['Investor Profiles','Detailed view of each investor'],
  profitloss:['Profit / Loss','Track financial performance over time'],
  dividend:  ['Dividend','Calculate & disburse investor dividends'],
};
...
async function renderDashboard() {
  try {
    const [sum, dirs] = await Promise.all([
      api(API.investors.summary),
      api(API.investors.getAll)
    ]);
    S.dirs = dirs;
    ...
    if (!dirs.length) { snap.innerHTML = '<div class="text-center py-6 text-navy/35 text-sm">No investors yet.</div>'; return; }
...
async function renderSettings() {
  try {
    S.dirs = await api(API.investors.getAll);
    ...
    document.getElementById('s-summary').textContent =
      `${S.dirs.length} investor${S.dirs.length!==1?'s':''} · ${fmt(totalInv)} total`;

    if (!S.dirs.length) {
      tbody.innerHTML = '<tr><td colspan="5" class="text-center py-10 text-navy/35 text-sm">No investors yet. Add one!</td></tr>';
      return;
    }
...
async function saveDir() {
  ...
  try {
    if (id) {
      await api(API.investors.update, 'POST', { 
        id: parseInt(id), 
        name, 
        email, 
        phone, 
      ...
      await api(API.investors.create, 'POST', { 
        name, 
        email, 
        phone, 
        investAmount,
      ...
      toast(`${name} added as investor!`, 'success');
    }
    // cancelEdit();
    renderSettings();
    ...
    btn.disabled = false;
    const btnText = id ? 'Save Changes' : 'Add Investor';
...
function editDir(id) {
  ...
  document.getElementById('form-title').textContent = 'Edit Investor';
...
function cancelEdit() {
  ...
  document.getElementById('form-title').textContent = 'Add Investor';
  document.getElementById('save-lbl').textContent   = 'Add Investor';
...
async function renderProfiles() {
    ...
    try {
        ...
        if (!S.dirs.length) {
          grid.innerHTML = '<div class="col-span-4 text-center py-12 text-navy/35 text-sm">No investors found.</div>';
          return;
    }
...
async function confirmInvest() {
  ...
  try {
    const res = await api(API.tx.invest, 'POST', { investor_id: modalTarget, amount: add, note });
    // Update local state
...
async function confirmWithdraw() {
  ...
  try {
    const res = await api(API.tx.withdraw, 'POST', { investor_id: modalTarget, amount: amt, note });
    if (d) { d.total_investment = res.total_investment; d.ownership_percent = res.ownership_percent; }
...
function openDelete(id) {
  modalTarget = id;
  const d = S.dirs.find(d => d.id == id);
  document.getElementById('del-msg').textContent = `This will deactivate "${d?.name}" as an investor. Their transaction history will be preserved.`;
  openModal('m-delete');
}

async function confirmDelete() {
  try {
    await api(API.investors.delete, 'POST', { id: modalTarget });
    toast('Investor deactivated.', 'success');
    closeModal('m-delete');
    renderSettings();
    renderDashboard();
  } catch(e) { toast(e.message, 'error'); }
...
async function calcDiv() {
  ...
  try {
    ...
    tbody.innerHTML = data.breakdown.map(b => `
      <tr>
        <td class="font-semibold">${b.investor_name}</td>
  ...
  try {
    ...
    tbody.innerHTML = history.map(h => `
      <tr>
        ...
        <td class="center">${h.investor_count}</td>
      ...
      <tr>
        <td>
          <div class="dir-name-cell">
            <div class="dir-av" style="background:${AV_COLORS[i % AV_COLORS.length]}">${d.investor_name[0]}</div>
            <span class="dir-name">${d.investor_name}</span>
          </div>
        </td>
