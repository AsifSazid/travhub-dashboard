    ...
    <div class="max-w-5xl mx-auto shadow-2xl main-container overflow-hidden">
        ...
        <div class="flex justify-between items-start mb-8 px-1">
            <div class="flex gap-4">
                <img src="logo.jpg" alt="Logo" class="h-14 w-auto" onerror="this.style.display='none'">
                <div>
            ...
            <div class="text-right">
                <div class="border-l-4 border-blue-600 pl-3">
                    <p class="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]"><span contenteditable="true">Airlines (PNR)</span></p>
                    <p class="text-3xl font-black text-slate-900 font-mono italic uppercase">
                        <span contenteditable="true"><?php echo val('booking_details.booking_reference_pnr'); ?></span>
                    </p>
                </div>
            </div>
        </div>
                ...
                <tbody class="text-[11px]">
                    ...
                    <tr class="border-b border-slate-50">
                        ...
                        <td class="py-2.5 font-bold text-slate-800 uppercase italic">
                            <span contenteditable="true"><?php echo nested_val($p, 'full_name'); ?></span>
                        </td>
                        <td class="py-2.5 font-mono text-slate-500">
                            <span contenteditable="true"><?php echo nested_val($p, 'ticket_number'); ?></span>
                        </td>
                        <td class="py-2.5 text-slate-500 italic">
                            
                            <span contenteditable="true"><?php echo nested_val($p, 'type'); ?></span>
                            </td>
        ...
        <div class="mb-8">
            ...
            <div class="no-print bg-slate-100 p-4 mb-4 rounded flex gap-3 items-center border border-slate-200">
                ...
                <button onclick="buildJourney()" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded text-sm font-bold uppercase transition-colors">Build Journey Block</button>
            </div>
                ...
                <tbody id="master-flight-body">
                    ...
                    <tr class="border-b border-slate-100 flight-row">
                       ...
                       <td class="p-3 text-center align-top opacity-30">
                            ...
                            <div class="text-[7px] font-bold uppercase"><span contenteditable="true"><?php echo nested_val($f, 'duration'); ?></span></div>
                        </td>
                        ...
                        <td class="p-3 align-top text-right pr-3">
                            <div class="text-[10px] font-bold text-slate-700 italic"><span contenteditable="true"><?php echo nested_val($f, 'class'); ?></span></div>
    ...
    <script>
        function calculateLayover(arrivalRow, departureRow) {
            try {
                ...
                const arrivalTimeStr = arrivalRow.querySelector('.arrival-cell .time-val').innerText;
                const departureDateStr = departureRow.querySelector('.departure-cell .date-val').innerText;
                const departureTimeStr = departureRow.querySelector('.departure-cell .time-val').innerText;
                const cityStr = arrivalRow.querySelector('.arrival-cell .city-val').innerText;

                const arrival = new Date(`${arrivalDateStr} ${arrivalTimeStr}`);
                const departure = new Date(`${departureDateStr} ${departureTimeStr}`);

                const diffMs = departure - arrival;
                if (isNaN(diffMs) || diffMs < 0) return `Connection in ${cityStr} — Layover: N/A`;

                const diffHrs = Math.floor(diffMs / 3600000);
                const diffMins = Math.round((diffMs % 3600000) / 60000);

                return `Connection in ${cityStr} — Layover: ${diffHrs}h ${diffMins}m`;
            } catch (e) {
        ...
        function buildJourney() {
            ...
            if (checkboxes.length === 0) {
                alert("Please select flights first!");
                return;
            }
            ...
            checkboxes.forEach((cb, index) => {
                ...
                if (index < checkboxes.length - 1) {
                    ...
                    const layoverText = calculateLayover(row, nextRow);
                    ...
                    connRow.innerHTML = `
                        ...
                        <td colspan="5" class="p-2 pl-3 italic text-slate-600 font-semibold">
                            <span contenteditable="true">${layoverText}</span>
                        </td>
                    `;
