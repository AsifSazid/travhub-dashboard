...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-print-color-adjust: exact; }
        ...
        header { 
            padding: 25px 40px; 
            background: #e2e8f0; /* Professional Light Gray */
...
<div class="container">
    <header>
        <div class="brand" style="display: flex; align-items: center; gap: 15px;">
            ...
            <div>
                <h1 contenteditable="true" style="font-size: 24px;">TravHub Global Limited</h1>
                <p style="font-size: 12px; opacity: 0.8;">
                    Confirmation: <strong contenteditable="true"><?php echo safeHtml($data['booking_id']); ?></strong> · 
                    PIN: <span contenteditable="true"><?php echo safeHtml($data['pin']); ?></span>
                </p>
            </div>
        </div>
        <div class="status-pill" contenteditable="true">✓ Paid In Full</div>
    </header>

    <div class="section">
        <div style="display: flex; justify-content: space-between; align-items: flex-start;">
            <div>
                <h2 class="section-title" contenteditable="true"><?php echo safeHtml($data['hotel_name']); ?></h2>
                ...
                if (!empty($fullAddress)) {
                    echo '<p style="color: var(--text-muted); font-size: 12px; max-width: 500px;" contenteditable="true">' . safeHtml(implode(', ', $fullAddress)) . '</p>';
                } else {
                    echo '<p style="color: var(--text-muted); font-size: 12px; max-width: 500px;" contenteditable="true">' . safeHtml($data['hotel_address'] ?? 'Address not available') . '</p>';
                }
                ?>
                
                <p style="margin-top: 5px; font-size: 12px;"><strong>Phone:</strong> <span contenteditable="true"><?php echo safeHtml($data['hotel_phone']); ?></span></p>
            </div>
    ...
    <div class="booking-hero">
        <div class="date-card">
            ...
            <div class="date-value" contenteditable="true"><?php echo formatDate($data['check_in_date']); ?></div>
            <div style="font-size: 10px; color: var(--text-muted);" contenteditable="true">After 2:00 PM</div>
        </div>
        <div class="date-card">
            <span class="date-label">Check-Out</span>
            <div class="date-value" contenteditable="true"><?php echo formatDate($data['check_out_date']); ?></div>
            <div style="font-size: 10px; color: var(--text-muted);" contenteditable="true">Before 12:00 PM</div>
        </div>
        <div class="date-card">
            <span class="date-label">Stay Details</span>
            <div class="date-value"><span contenteditable="true"><?php echo $nights; ?></span> Nights</div>
            <div style="font-size: 10px; color: var(--text-muted); font-weight: 600;">
                <span contenteditable="true"><?php echo safeHtml($data['total_rooms']); ?></span> Room(s) | 
                <span contenteditable="true"><?php echo is_array($data['guest_names']) ? count($data['guest_names']) : '1'; ?></span> Guest(s)
            </div>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <table class="data-table">
            ...
            <tbody>
                <tr>
                    <td><strong style="font-size: 13px;" contenteditable="true"><?php echo safeHtml(is_array($data['guest_names']) ? ($data['guest_names'][0] ?? 'N/A') : $data['guest_names']); ?></strong></td>
                    <td contenteditable="true">
                        ...
                        if (is_array($data['guest_names'])) {
                            ...
                            echo "No additional guests";
                        }
                        ?>
                    </td>
                    <td><strong contenteditable="true"><?php echo safeHtml($data['total_rooms']); ?> Room(s)</strong></td>
                </tr>
            </tbody>
        </table>
    </div>
    ...
    <div class="section">
        ...
        <p style="font-weight: 700; font-size: 15px; color: var(--accent);" contenteditable="true"><?php echo safeHtml($data['room_type']); ?></p>
        <p style="font-size: 12px; color: var(--text-muted); margin-bottom: 10px;" contenteditable="true">Plan: <strong><?php echo safeHtml($data['meal_plan']); ?></strong></p>
        <div>
            <span class="pill" contenteditable="true">Free High-Speed WiFi</span>
            <span class="pill" contenteditable="true">Air Conditioning</span>
            <span class="pill" contenteditable="true">Private Bathroom</span>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div>
                ...
                <p style="font-size: 11px; color: var(--text-muted);" contenteditable="true"><?php echo safeHtml($data['cancellation_policy']); ?></p>
            </div>
            <div>
                <h4 style="font-size: 10px; text-transform: uppercase; color: var(--primary);">Important</h4>
                <p style="font-size: 11px; color: var(--text-muted);" contenteditable="true">Valid passport and visa required for check-in.</p>
            </div>
        </div>
    </div>
    ...
    <div class="section" style="border: none;">
        ...
        <div class="pricing-table">
            <div class="price-row">
                ...
                <span><span contenteditable="true"><?php echo safeHtml($data['currency']); ?></span> <span contenteditable="true"><?php echo number_format($raw_price * 0.83, 2); ?></span></span>
            </div>
            <div class="price-row">
                <span>Service Charge & Taxes</span>
                <span><span contenteditable="true"><?php echo safeHtml($data['currency']); ?></span> <span contenteditable="true"><?php echo number_format($raw_price * 0.17, 2); ?></span></span>
            </div>
            <div class="price-row price-total">
                <span>Total Amount Paid</span>
                <span><span contenteditable="true"><?php echo safeHtml($data['currency']); ?></span> <span contenteditable="true"><?php echo number_format($raw_price, 2); ?></span></span>
            </div>
        </div>
    </div>
    ...
    <footer>
        <div>
            <strong contenteditable="true">TravHub Global Limited (Support)</strong><br>
            <span contenteditable="true">24/7 Helpline: +880 1611 482 773</span>
        </div>
        <div style="text-align: right;">
            <span contenteditable="true">Digital Voucher ID: <?php echo strtoupper(substr(md5($data['booking_id']), 0, 12)); ?></span><br>
            <em contenteditable="true">travhub.com.bd</em>
        </div>
    </footer>
</div>
