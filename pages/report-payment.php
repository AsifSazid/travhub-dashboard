...
<head>
    ...
    <title>Payment Report — TravHub</title>
...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-900"><i class="fas fa-money-bill-wave text-red-600 mr-2"></i>Payment Report</h1>
            <p class="text-sm text-gray-500 mt-1">All cash paid out of your accounts</p>
        </div>
        <div class="relative" id="exportDropdownWrap">
            <button onclick="toggleExportMenu()" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium flex items-center gap-2">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Date From</label>
                <input type="date" id="f-date-from" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-400"></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Date To</label>
                <input type="date" id="f-date-to" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-400"></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Quick Month</label>
                <input type="month" id="f-month" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-400"></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Account</label>
                <select id="f-account" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-400"><option value="">All Accounts</option></select></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Account Head</label>
                <select id="f-account-head" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-400">
                    <option value="">All Types</option>
                    <option value="accounts_payable">Vendor Payment</option>
            ...
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Search</label>
                <input type="text" id="f-search" placeholder="Account / Description..." class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-400"></div>
        </div>
        <div class="flex gap-2 mt-3">
            <button onclick="applyFilters()" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium"><i class="fas fa-search mr-1"></i> Apply</button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p class="text-2xl font-bold text-red-600 mt-1" id="sum-amount">—</p>
        </div>
    </div>

    <div id="loadingBar" class="hidden text-center py-8"><i class="fas fa-spinner fa-spin text-2xl text-red-500"></i></div>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-medium text-gray-500 uppercase text-xs">Paid To</th>
...
async function loadList() {
    setLoading(true);
    try {
        // Payments are cash_out rows within action=detail; filtered client-side
        ...
        document.getElementById('sum-amount').textContent = fmt(rows.reduce((s,r)=>s+parseFloat(r.cash_out||0),0));
        ...
        document.getElementById('list-tbody').innerHTML = rows.map(row => `
            ...
            </tr>`).join('') || '<tr><td colspan="6" class="px-4 py-6 text-center text-gray-400">No payments found</td></tr>';
        ...
        pag.innerHTML = `<span>Page ${d.page} of ${d.pages}</span>
            <div class="flex gap-2">
                ...
                ${d.page < d.pages ? `<button onclick="goPage(${d.page+1})" class="px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700">Next</button>` : ''}
            </div>`;
...
async function exportData(type = 'csv') {
    ...
    try {
        ...
        const filename = `payment-report-${dateSuffix}`;
        const allRows = [];
        let page = 1, pages = 1;
        do {
        ...
        if (type === 'csv') {
            ...
            exportPDF('Payment Report', rows, filename);
        }
    } finally { setLoading(false); }
}
