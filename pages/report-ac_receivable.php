...
<head>
    ...
    <title>Accounts Receivable — TravHub</title>
...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">
                <i class="fas fa-hand-holding-usd text-indigo-600 mr-2"></i>Accounts Receivable
            </h1>
            <p class="text-sm text-gray-500 mt-1">how much each client still owes you</p>
        </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <div>
                ...
                <input type="date" id="f-date-from" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">Date To</label>
                <input type="date" id="f-date-to" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">Amount Min</label>
                <input type="number" step="0.01" id="f-amount-min" placeholder="0.00" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">Amount Max</label>
                <input type="number" step="0.01" id="f-amount-max" placeholder="0.00" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400">
            </div>
            <div class="col-span-2">
                <label class="block text-xs font-medium text-gray-500 mb-1">Search Client</label>
                <input type="text" id="f-search" placeholder="Client name / ID..." class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-400">
            </div>
        </div>
        <div class="flex gap-2 mt-3">
            ...
            <div class="ml-auto flex gap-2">
                <button onclick="setView('list')" id="v-list" class="px-3 py-2 rounded-lg text-sm font-medium bg-indigo-600 text-white">Client List</button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Clients with Outstanding Balance</p>
            <p class="text-2xl font-bold text-gray-800 mt-1" id="sum-client-count">—</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Total Receivable</p>
            <p class="text-2xl font-bold text-indigo-600 mt-1" id="sum-total-receivable">—</p>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right font-medium text-gray-500 uppercase text-xs">Total Sales</th>
                        <th class="px-4 py-3 text-right font-medium text-gray-500 uppercase text-xs">Paid/Reversed</th>
                        <th class="px-4 py-3 text-right font-medium text-gray-500 uppercase text-xs">Receivable Balance</th>
    ...
    <div id="view-detail" class="hidden bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="p-3 bg-gray-50 border-b border-gray-100 text-xs text-gray-500" id="detail-client-label">Select a client from the List view to see its transactions, or use Search above.</div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
...
<script>
const API = "<?php echo $ip_port; ?>/api/reports/receivable_v2/endpoints.php";
...
function setView(v) {
    ...
    ['list','detail'].forEach(x => {
        document.getElementById(`v-${x}`).className = `px-3 py-2 rounded-lg text-sm font-medium ${x===v?'bg-indigo-600 text-white':'bg-gray-200 text-gray-700'}`;
    });
    if (v === 'list') loadList();
}
...
async function loadList() {
    ...
    try {
        ...
        document.getElementById('sum-client-count').textContent = d.summary.total_clients;
        document.getElementById('sum-total-receivable').textContent = fmt(d.summary.total_receivable);

        const tb = document.getElementById('list-tbody');
        tb.innerHTML = (d.rows||[]).map(row => `
            <tr class="hover:bg-gray-50 cursor-pointer" onclick="openClientDetail('${row.user_sys_id}','${(row.user_name||'').replace(/'/g,"\\'")}')">
                <td class="px-4 py-3 font-medium text-gray-800">${row.user_name||row.user_sys_id}</td>
                <td class="px-4 py-3 text-right text-gray-600">${fmt(row.total_sales_debits)}</td>
                <td class="px-4 py-3 text-right text-green-700">${fmt(row.total_receives_and_reversals)}</td>
                <td class="px-4 py-3 text-right font-semibold text-indigo-700">${fmt(row.receivable_balance)}</td>
            ...
            </tr>`).join('') || '<tr><td colspan="6" class="px-4 py-6 text-center text-gray-400">No outstanding receivables</td></tr>';

        const pag = document.getElementById('list-pagination');
        pag.innerHTML = `<span>Page ${d.page} of ${d.pages} (${d.total} clients)</span>
...
function openClientDetail(clientId, clientName) {
    ...
    document.getElementById('f-search').value = clientName;
    setView('detail');
    loadDetail();
}

async function loadDetail() {
    if (!selectedClientId) { document.getElementById('detail-tbody').innerHTML = '<tr><td colspan="5" class="px-4 py-6 text-center text-gray-400">Select a client from the List view first</td></tr>'; return; }
    setLoading(true);
    try {
        document.getElementById('detail-client-label').textContent = `Showing all accounts_receivable entries for: ${document.getElementById('f-search').value}`;
        const r = await fetch(`${API}?action=detail&user_sys_id=${encodeURIComponent(selectedClientId)}`);
        const d = await r.json();
        if (!d.success) return;
        document.getElementById('detail-tbody').innerHTML = (d.rows||[]).map(row => {
            ...
            return `<tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3"><span class="text-xs font-bold ${isDebit?'text-green-600':'text-indigo-600'}">${isDebit?'DEBIT (paid)':'CREDIT (purchase)'}</span></td>
...
async function exportData(type = 'csv') {
    ...
    try {
        ...
        if (currentView === 'list') {
            ...
            filename = `receivable-list-${dateSuffix}`;
            rows = [
                ['Accounts Receivable — Client List', '', '', '', ''],
                ['Total Clients', d.summary.total_clients, '', '', ''],
                ['Total Receivable', d.summary.total_receivable, '', '', ''],
                ['', '', '', '', ''],
                ['Client', 'Total Sales', 'Paid/Reversed', 'Receivable Balance', 'Entries'],
                ...(d.rows||[]).map(row => [row.user_name||row.user_sys_id, parseFloat(row.total_sales_debits), parseFloat(row.total_receives_and_reversals), parseFloat(row.receivable_balance), row.entry_count]),
            ];
            ...
            const d = await r.json();
            if (!d.success) { alert('Export failed'); return; }
            filename = `receivable-detail-${dateSuffix}`;
            rows = [
        ...
        if (type === 'csv') {
            ...
            exportPDF('Accounts Receivable Report', rows, filename);
        }
    } finally { setLoading(false); }
}
