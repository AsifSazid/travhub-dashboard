...
<head>
    ...
    <title>Purchase Report — TravHub</title>
...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-900"><i class="fas fa-shopping-cart text-emerald-600 mr-2"></i>Purchase Report</h1>
            <p class="text-sm text-gray-500 mt-1">All vendor purchases (net of refunds)</p>
        </div>
        <div class="relative" id="exportDropdownWrap">
            <button onclick="toggleExportMenu()" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium flex items-center gap-2">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-4 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Date From</label>
                <input type="date" id="f-date-from" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-400"></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Date To</label>
                <input type="date" id="f-date-to" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-400"></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Vendor</label>
                <select id="f-vendor" multiple size="1" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-400"><option value="">All Vendors</option></select></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Work</label>
                <select id="f-work" multiple size="1" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-400"><option value="">All Works</option></select></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Amount Min</label>
                <input type="number" step="0.01" id="f-amount-min" placeholder="0.00" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-400"></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Amount Max</label>
                <input type="number" step="0.01" id="f-amount-max" placeholder="0.00" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-400"></div>
            <div><label class="block text-xs font-medium text-gray-500 mb-1">Paid Status</label>
                <select id="f-is-paid" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-400">
            ...
            <div class="col-span-2"><label class="block text-xs font-medium text-gray-500 mb-1">Search</label>
                <input type="text" id="f-search" placeholder="Purpose / Vendor / Work..." class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-emerald-400"></div>
        </div>
        <div class="flex gap-2 mt-3">
            <button onclick="applyFilters()" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium"><i class="fas fa-search mr-1"></i> Apply</button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Total Purchase Amount (net of refunds)</p>
            <p class="text-2xl font-bold text-emerald-600 mt-1" id="sum-amount">—</p>
        </div>
    </div>

    <div id="loadingBar" class="hidden text-center py-8"><i class="fas fa-spinner fa-spin text-2xl text-emerald-500"></i></div>
...
<script>
const API = "<?php echo $ip_port; ?>/api/reports/purchase_v2/endpoints.php";
let currentPage = 1;
...
function buildParams() {
    ...
    [...document.getElementById('f-vendor').selectedOptions].forEach(o => { if (o.value) p.append('vendor[]', o.value); });
...
function clearFilters() {
    ...
    document.getElementById('f-vendor').selectedIndex = -1;
...
async function loadFilterOptions() {
    try {
        ...
        (d.vendors||[]).forEach(v => vSel.innerHTML += `<option value="${v.user_sys_id}">${v.user_name||v.user_sys_id}</option>`);
    ...
    try {
        ...
        tb.innerHTML = (d.rows||[]).map(row => `
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-right font-medium ${row.type==='credit'?'text-rose-500':'text-emerald-700'}">${row.type==='credit'?'-':''}${fmt(row.amount)}</td>
            </tr>`).join('') || '<tr><td colspan="6" class="px-4 py-6 text-center text-gray-400">No purchases found</td></tr>';
        ...
        pag.innerHTML = `<span>Page ${d.page} of ${d.pages} (${d.total} entries)</span>
            <div class="flex gap-2">
                ...
                ${d.page < d.pages ? `<button onclick="goPage(${d.page+1})" class="px-3 py-1 bg-emerald-600 text-white rounded hover:bg-emerald-700">Next</button>` : ''}
            </div>`;
...
async function exportData(type = 'csv') {
    ...
    try {
        ...
        const filename = `purchase-report-${dateSuffix}`;
        const rows = [
            ['Purchase Report', '', '', '', '', ''],
            ['Total Entries', d.summary.total_count, '', '', '', ''],
            ['Total Amount', d.summary.total_amount, '', '', '', ''],
            ['', '', '', '', '', ''],
            ['Date', 'Vendor', 'Purpose', 'Work', 'Status', 'Amount'],
        ...
        if (type === 'csv') {
            ...
            exportPDF('Purchase Report', rows, filename);
        }
    } finally { setLoading(false); }
}
