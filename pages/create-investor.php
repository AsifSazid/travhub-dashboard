...
$storeInvestorApi = $ip_port . "api/investors/store.php";
...
<body class="bg-gray-50 font-sans">
            ...
            <div class="bg-white rounded-lg shadow p-4">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        <div>
                            <div class="flex items-center mb-2">
                                ...
                                <div>
                                    <h1 class="text-2xl font-bold text-gray-800">Add New Investor</h1>
                                    <p class="text-gray-600 text-sm mt-1">Fill in the details below to add a new investor to the system</p>
                                </div>
                            </div>
                        </div>
                        <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 min-w-[200px]">
                            <div class="text-xs text-gray-500 font-medium uppercase tracking-wider mb-1">Investor ID</div>
                            <div class="text-lg font-semibold text-gray-800" id="previewId">IVS-XXXXXXX</div>
                        </div>
                    </div>
                </div>
                ...
                <form id="investorForm" class="space-y-6">
                    ...
                    <div class="form-section p-5">
                        <h2 class="section-title">Investor Type</h2>
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mt-4">
                            ...
                            foreach ($investorTypes as $value => $info):
                            ?>
                                ...
                                <div class="grid grid-cols-1 gap-4">
                                    <div>
                                        <label for="designation" class="form-label mb-1">
                                            Percentage <span class="required-star">*</span>
                                        </label>
                                        <div class="relative">
                                    ...
                                    <div>
                                        <label for="dateOfJoin" class="form-label mb-1">
                                            Date of Investing <span class="required-star">*</span>
                                        </label>
                                        <div class="relative">
                                            <input type="date" id="dateOfJoin" name="date_of_investing"
    ...
    <script>
        const API_URL_FOR_CLIENT_STORE = "<?php echo $storeInvestorApi; ?>";
        ...
        document.getElementById('investorForm').addEventListener('submit', async function (e) {
            ...
            const phone = this.primary_phone.value.trim();
            const email = this.primary_email.value.trim();
            const percentage = this.percentage.value.trim();
            const dateJoin = this.date_of_investing.value;
        
            if (!fullName || !phone || !email || !percentage || !dateJoin) {
            ...
            if (droppedFiles.length === 0) {
                if (!confirm('No files uploaded. Continue without files?')) return;
            }
            ...
            const investorData = {
                ...
                date_of_investing: formData.get('date_of_investing'),
                blood_group: formData.get('blood_group'),
                status: 'active',
        
                phone: {
                    primary: formData.get('primary_phone'),
                    secondary: formData.getAll('secondary_phone_number[]').map((num, i) => ({
                        type: formData.getAll('secondary_phone_type[]')[i],
                        number: num
                    }))
                },
                ...
                email: {
                    ...
                    secondary: formData.getAll('secondary_email_address[]').map((mail, i) => ({
                        type: formData.getAll('secondary_email_type[]')[i],
                        address: mail
                    }))
                },
        
                address: {
                    line1: formData.get('address_line_1'),
                    line2: formData.get('address_line_2'),
                ...
                emergency_contact: {
                    ...
                    address: {
                        line1: formData.get('emergency_address_line_1'),
                        line2: formData.get('emergency_address_line_2'),
                        city: formData.get('emergency_city'),
                        state: formData.get('emergency_state'),
                        zip: formData.get('emergency_zip_code')
                    }
                }
            };
            ...
            uploadData.append('investor_data', JSON.stringify(investorData));
            ...
            // ---------- API CALL ----------
            try {
                const res = await fetch(API_URL_FOR_CLIENT_STORE, {
                    method: 'POST',
                    body: uploadData
                });
                ...
                if (result.success) {
                    showMessage('Investor added successfully', 'success');
                    setTimeout(resetForm, 1500);
                } else {
                    showMessage(result.message || 'Failed to save investor', 'error');
                }
        ...
        function resetForm() {
            if (confirm('Are you sure you want to reset the form? All entered data will be lost.')) {
                ...
                // Set default investor type to permanent
