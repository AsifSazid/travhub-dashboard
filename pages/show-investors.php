...
// $showInvestorApi = $ip_port . "api/investors/show.php";
$getInvestorApi = $ip_port . "api/investors/get-investor.php?investor_id=$investorId";
...
<head>
    ...
    <title>Investor Portfolio - <?php echo htmlspecialchars($investorId ?? 'Investor'); ?></title>
...
<body class="bg-gray-50 font-sans">
    ...
    <main id="mainContent" class="pl-64 transition-all duration-300">
        <div class="p-6">
            ...
            <div id="loadingContainer" class="bg-white rounded-lg shadow p-8 text-center">
                ...
                <p class="mt-4 text-gray-600">Loading investor information...</p>
            </div>
                ...
                <div class="portfolio-header mb-6 p-8 relative">
                    <div class="flex flex-col md:flex-row items-center md:items-start">
                        ...
                        <div class="flex-1 text-center md:text-left">
                            <h1 id="investorName" class="text-3xl md:text-4xl font-bold mb-2">Investor Name</h1>
                            <h2 id="investorDesignation" class="text-xl md:text-2xl text-blue-100 mb-3">Designation</h2>
                            <div class="flex flex-wrap justify-center md:justify-start gap-4 text-blue-100 mb-6">
                                <div class="flex items-center">
                                    <i class="fas fa-id-card mr-2"></i>
                                    <span id="investorId">EMP-0000000</span>
                                </div>
                                <div class="flex items-center">
                                    <i class="fas fa-calendar-alt mr-2"></i>
                                    <span id="investorJoinDate">Investing Date</span>
                                </div>
                            </div>
                            
                            <!-- Contact Info -->
                            <div class="flex flex-wrap justify-center md:justify-start gap-4">
                                <a id="investorEmail" href="#" class="flex items-center bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg transition">
                                    <i class="fas fa-envelope mr-2"></i>
                                    <span>Email</span>
                                </a>
                                <a id="investorPhone" href="#" class="flex items-center bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg transition">
                                    <i class="fas fa-phone mr-2"></i>
                                    <span>Phone</span>
                                </a>
                                <a id="investorLocation" href="#" class="flex items-center bg-white/20 hover:bg-white/30 px-4 py-2 rounded-lg transition">
                        ...
                        <div class="info-card personal p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <div id="investorType" class="detail-value">Not available</div>
                                </div>
                            </div>
                            <div class="mt-6">
                        ...
                        <div class="info-card p-6">
                            ...
                            <div class="space-y-4">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-4">
                                        <div class="detail-value">Investor Created</div>
            ...
            <div id="errorContainer" class="bg-white rounded-lg shadow p-8 text-center hidden">
                ...
                <h3 class="text-xl font-bold text-gray-800 mt-4">Error Loading Investor Data</h3>
                <p id="errorMessage" class="text-gray-600 mt-2">Could not load investor information. Please try again.</p>
    ...
    <script>
        const GET_DIRECTOR_INFO_API = "<?php echo $getInvestorApi; ?>";
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Function to fetch and display investor data
            async function loadInvestorData() {
                try {
                    ...
                    const response = await fetch(GET_DIRECTOR_INFO_API);
                    ...
                    if (data.success && data.investor) {
                        ...
                        // Parse JSON fields
                        const investmentDate = parseJsonField(investor.date_of_investing) || {};
                        const emailData = parseJsonField(investor.email) || {};
                        const phoneData = parseJsonField(investor.phone) || {};
                        const addressData = parseJsonField(investor.address) || {};
                        const basicInfoData = parseJsonField(investor.basic_info) || {};
                        const emergencyContactData = parseJsonField(investor.emergency_contact) || {};
                        const companyInfoData = parseJsonField(investor.company_related_info) || {};
                        const metaData = parseJsonField(investor.meta_data) || {};
                        
                        // Set profile photo (if available)
                        const profilePhoto = document.getElementById('profilePhoto');
                        if (investor.image_name) {
                            try {
                                const imageData = JSON.parse(investor.image_name);
                        ...
                        statusBadge.textContent = investor.status || 'Unknown';
                        statusBadge.className = `status-badge ${investor.status === 'active' ? 'status-active' : 'status-inactive'}`;
                        
                        // Header section
                        document.getElementById('investorName').textContent = investor.name || 'Unknown';
                        document.getElementById('investorId').textContent = investor.sys_id || 'DIR-0000000';
                        document.getElementById('investorJoinDate').textContent = investor.date_of_investing || 'No Data Found';
                        
                        // Designation
                        const designation = companyInfoData.designation || 'Investor';
                        document.getElementById('investorDesignation').textContent = designation;

                        // Contact links in header
                        const emailLink = document.getElementById('investorEmail');
                        ...
                        const phoneLink = document.getElementById('investorPhone');
                        ...
                        const locationLink = document.getElementById('investorLocation');
                        ...
                        // About section
                        document.getElementById('sysId').textContent = investor.sys_id || 'EMP-0000000';
                        ...
                        document.getElementById('investorType').textContent = investor.type ? investor.type.charAt(0).toUpperCase() + investor.type.slice(1) : 'Not available';
                        ...
                        if (investor.department_name) {
                            skillsContainer.innerHTML = '';
                            // Add department-specific skills
                            let skills = [];
                            
                            if (investor.department_name === 'IT') {
                                skills = ['Software Development', 'System Analysis', 'Database Management', 'Web Technologies', 'Problem Solving'];
                            } else if (investor.department_name === 'HR') {
                                skills = ['Recruitment', 'Investor Relations', 'Training & Development', 'HR Policies', 'Communication'];
                            } else if (investor.department_name === 'Finance') {
                        ...
                        throw new Error('Invalid investor data received');
                    }
                } catch (error) {
            ...
            // Load investor data on page load
            loadInvestorData();
            
            // Retry button functionality
            retryButton.addEventListener('click', loadInvestorData);
            ...
            document.getElementById('editButton').addEventListener('click', function() {
                alert('Edit functionality would open a form to update investor information.');
