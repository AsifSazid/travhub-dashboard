...
$storeDirectorApi = $ip_port . "api/directors/store.php";
...
<body class="bg-gray-50 font-sans">
    ...
    <main id="mainContent" class="pt-16 pl-64 lg:mt-16 transition-all duration-300">
        <div class="p-6">
            <div class="bg-white rounded-lg shadow p-4">
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        <div>
                            <div class="flex items-center mb-2">
                                ...
                                <div>
                                    <h1 class="text-2xl font-bold text-gray-800">Add New Director</h1>
                                    <p class="text-gray-600 text-sm mt-1">Fill in the details below to add a new director to the system</p>
                                </div>
                            </div>
                        </div>
                        <div class="bg-gray-50 p-4 rounded-lg border border-gray-200 min-w-[200px]">
                            <div class="text-xs text-gray-500 font-medium uppercase tracking-wider mb-1">Director ID</div>
                            <div class="text-lg font-semibold text-gray-800" id="previewId">DIR-XXXXXXX</div>
                        </div>
                    </div>
                </div>
                ...
                <!-- Director Form -->
                <form id="directorForm" class="space-y-6">
    ...
    <script>
        const API_URL_FOR_DIRECTOR_STORE = "<?php echo $storeDirectorApi; ?>";
        ...
        document.getElementById('directorForm').addEventListener('submit', async function (e) {
            ...
            /* ================= SECONDARY PHONES ================= */
            formData.getAll('secondary_phone_type[]').forEach((type, i) => {
                data.phone.secondary_no.push({
                    type,
                    number: formData.getAll('secondary_phone_number[]')[i] || ''
                });
            });
        
            /* ================= SECONDARY EMAILS ================= */
            formData.getAll('secondary_email_type[]').forEach((type, i) => {
                data.email.secondary.push({
                    type,
                    address: formData.getAll('secondary_email_address[]')[i] || ''
                });
            });
            ...
            uploadFormData.append('director_data', JSON.stringify(data));
        
            try {
                const res = await fetch(API_URL_FOR_DIRECTOR_STORE, {
                    method: 'POST',
                    body: uploadFormData
                });
                ...
                if (result.success) {
                    showMessage('Director added successfully', 'success');
                    resetForm();
                } else {
                    showMessage(result.message || 'Failed to add director', 'error');
                }
                ...
                submitBtn.innerHTML = '<i class="fas fa-user-plus mr-2"></i> Add Director';
            }
        });
        ...
        function resetForm() {
            if (confirm('Are you sure you want to reset the form? All entered data will be lost.')) {
                ...
                // Set default director type to permanent
